...
snapshot_start_date ='2020-07-02'
snapshot_end_date = '2021-03-02'
...
#NPS_w1_FY21
NPS_w1w2_FY21_Q3 = pd.read_excel(outDir + 'survey/fy21_Q3_all_invited20210310.xlsx')

# NPS_w1w2_FY21_Q3.dropna(axis = 0, how ="all", inplace = True)


#NPS_w1_FY2_Too soon list
too_soon = pd.read_excel('C://Users//wanthana.j.app//OneDrive - New Zealand Trade and Enterprise//Survey//Wave1_FY21//20-11-25 FY21_w1.xlsx', sheet_name='Too Soon List')
...
# %%
#moved to survey_export_tracker.ipynb

# #add NPS to the wide format
# NPS_w1w2_FY21_Q3 = NPS_w1w2_FY21_Q3.rename(columns={"Organisation_Name": 'Organisation Legal Name_'})
# NPS_and_egm =  pd.merge(NPS_w1w2_FY21_Q3, df_compared, how = "left", on = "Organisation Legal Name_")

# #save output to file
# outfilePath = outDir + "survey/w_nps" + exported_date + ".csv"
# (NPS_and_egm.replace('\n',' ', regex=True)
#     .to_csv(outfilePath, sep = ",",  index = False, header = True , encoding = "utf-8", line_terminator='\n')
# )
...
# %%
# Prepare Data
#exclude those with no Sector speci
df_raw = pd.DataFrame(df_compared.query("Sector_ != 0"))
...
group_by_feature = "Sector_"#"cluster_egm_chng_"

df = df_raw[['actions_chng', 'scv_actions_chng','cluster_egm_chng_', 'cluster_prev', 'cluster_aftr', 'Sector_', 'cohort_by_outcome']].groupby(group_by_feature).apply(lambda x: x.mean())
...
ax.set_yticklabels(df[group_by_feature], fontdict={'horizontalalignment': 'right'})
ax.set_xlim(-10, 15)
plt.show()
print(df)
...
#modify this part of source code to the wanted format (long or wide)
df =  (svc_compared.groupby(['svc_ori', 'svc_name'])["svc_count"].sum()
        #.sort_index(level= 1, ascending = False)
        .reset_index()
        .assign(snapshot = lambda x: x["svc_name"].str[-4:])  
        #uncomment this part to reshape to wide format
        .pivot(index = "svc_ori", columns  = "snapshot" ,values = "svc_count").reset_index()
        .assign(diff = lambda x: x["aftr"] - x["prev"])      
        .assign(diff_pct = lambda x: (x["aftr"] - x["prev"]) * 100/x["prev"])      
        .sort_values("aftr", ascending = False)
